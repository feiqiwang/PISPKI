...
###################################################################
        print("This is a data iter program, the ligand dataset will be collected by molecule types")
        ...
        self.binding_index_dict = {} #Save the binding place by dict and find it by molecule index
###################################################################
        ...
        if (os.path.isdir(path + "/" + os.listdir(path)[0] + "/" + os.listdir(path+"/"+os.listdir(path)[0])[0])):
            for pAC in os.listdir(path):
                self.ProteinAc = pAC
                self.molecules_type_collection(path+"/"+pAC)             
            for pAC in os.listdir(path):
                self.ProteinAc = pAC
                self.creatDataset(path+"/"+pAC)
        else:         
            self.ProteinAc = path.split('/')[-1]
            self.molecules_type_collection(path)
    ...
    def creatDataset(self,path):
        ...
        for ligand_index in os.listdir(path):
            mol_file = open(path + "/" + ligand_index.replace("\n","") + "/ligand.mol2")
            ...
            #Get the atom numer
            mol_line = mol_file.readline()
            molecule_index = mol_line.split("_")[1]
            

            if (molecule_index not in read_mol_set) and (molecule_index in self.binding_index_dict.keys()):
                read_mol_set.add(molecule_index)
                mol_line = mol_file.readline()
            ...
            ########Create orignal data iter#########
                ...
                for i in range(label_tensor.shape[0]):
                    if i+1 in self.binding_index_dict[molecule_index]:
                        ...
                        self.ligand_feature_list_positive[-1][i]=self.ligand_feature_list_positive[-1][i] * 1000 ####
                        ...
                        self.ligand_feature_list_negative[-1][i] = self.ligand_feature_list_negative[-1][i] * 1000 ####
    ...
    def DataLoader_test(self, batch_size):
        np = len(self.ligand_structure_test_list_positive)
        nn = len(self.ligand_structure_test_list_negative)
        n = min([np,nn])
    ...
    def molecules_type_collection(self,path):
        ...
        for ligand_index in os.listdir(path):
            ...
            bind_file = open(path + "/" + ligand_index.replace("\n","") + "/EXPORTINTS=" + ligand_index.replace("\n",""))
            ...
            if len(bind_index)!= 0:
                try:
                    ...
                    self.binding_index_dict[molecule_index].union(bind_index)

                except:

                        self.binding_index_dict[molecule_index] = set(bind_index)
...
##dataset = ligandDataset(path,mode="bpd")
##
##conf_iter = dataset.DataLoader_confuse(16,0.5)
##
